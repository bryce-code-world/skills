...
| AI 执行任务 | `TASK-XX` | S12 拆出的单个可验证业务行为；每个 TASK 只绑定一个责任服务或明确的调用适配层 |
...
一个 `WP-XX` 可以覆盖跨服务的完整业务能力，但一个 `TASK-XX` 只能修改和验证一个责任服务或明确的调用适配层。跨服务闭环必须拆成有序依赖链，分别固定服务、文件白名单、测试命令、Review 责任和失败边界；Proto、生成物和 clients 不能无说明地混入跨服务 TASK。
