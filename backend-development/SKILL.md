...
5. S1～S12 先完成上游逐项追溯，整理 AI 直接修复项、冲突、遗漏和最小人工决策，形成可定位的阶段评审包。
6. 满足退出条件后标记 `REVIEW_READY`。人在评审包形成前给出的笼统确认不能解释为放行；设计和协议只有在人确认对应评审包版本后才能标记 `FROZEN`。
...
下游发现已冻结结论需要变化时，先记录人的决定、原冻结基线和影响清单，并将最早受影响步骤标为 `CHANGE_PENDING`。再按上游到下游顺序更新和重新放行。

决策记录必须先于派生文档修改，保证中断后能够恢复。
