...
| AI 执行任务 | `WP-XX-TASK-YY` | S12 拆出的单个可验证业务行为；任务序号只在所属 WP 内递增，每个 TASK 只绑定一个清晰、可独立交付的责任单元或明确的调用适配层，并以一个主入口/独立闭环为颗粒度 |
...
| P5 AI 执行计划、Review 与交付 | S12～S15 | 任务已拆为可验证的 `WP-XX-TASK-YY`，实现、三级 Review、联调和验收证据闭环 |
...
设计、协议、开发和 Review 模式每次只推进一个工作切片或一个 `WP-XX-TASK-YY`。工作切片是当前步骤内一个可独立确认的子业务、流程簇、交付物或问题批次，不新增全局编号和永久文档。
...
S13 关闭每个 `WP-XX-TASK-YY` 前必须完成任务自检：
...
S12 的任务登记与依赖必须只有一个权威入口。默认每个任务使用 `WP-XX-TASK-YY`：`XX` 复用所属工作包编号，`YY` 只在该 WP 内从 `01` 递增；依赖必须引用完整任务 ID，因此一个 WP 的拆分或插入不会重排其他 WP。每个可执行任务必须归属一个 WP；跨 WP 的协调能力应建立明确的协调 WP 或归属 WP，不创建无归属的全局任务表。已存在的扁平 `TASK-XX` 作为历史兼容保留，只有人明确授权迁移时才批量改号并维护旧 ID 映射。

一个 `WP-XX` 可以覆盖跨服务、跨模块或跨组件的完整业务能力，但一个 `WP-XX-TASK-YY` 只能修改和验证一个清晰、可独立交付的责任单元或明确的调用适配层。责任单元按项目架构确定：微服务可以是服务，单体可以是模块、组件或分层；必要时也可以是独立生成物或客户端适配层。跨责任单元闭环必须拆成有序依赖链，分别固定责任边界、文件白名单、测试命令、Review 责任和失败边界；Proto、生成物和 clients 不能无说明地混入多个责任单元。
...
1. 一个会话只处理一个工作切片或一个 `WP-XX-TASK-YY`。切片完成后更新 `Progress.md` 和最小续跑检查点，不自动开始下一切片。
...
- 当前模式、迭代、研发大阶段、流程步骤、`WP-XX` 和 `WP-XX-TASK-YY`；
