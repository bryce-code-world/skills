...
3. 按 [platform-format-contract.md](platform-format-contract.md) 将 Markdown 语义映射为平台原生结构或稳定富文本。微信公众号不得接受导入器自动产生的蓝底标题块和不必要目录；优先使用普通深色标题、少量暖色编号或细线与移动端留白。
4. 字段不明确、存在多个同名账号或页面进入风控时停止，让用户处理。
5. 填入一次内容并保存一次草稿。修正既有草稿时覆盖同一草稿，不新建重复草稿。
6. 跳转或打开草稿箱，使用可见标题和正文开头确认草稿真实存在。
7. 打开草稿详情或预览，检查目录、标题、段落、图片、代码和适用终端呈现。微信公众号同时检查首屏、手机宽度、蓝底标题块和不必要目录。
8. 分别记录内容确认、排版确认和未确认字段；不为了“全自动”点击含义不清的控件。
...
- 是否已经按格式契约检查真实渲染；未检查时是否明确记录“草稿已保存，排版未验收”；
- 微信公众号是否默认无目录、无蓝底标题块，并完成移动端预览；
- 是否存在人工处理项；
- 是否始终没有触发公开发布。
