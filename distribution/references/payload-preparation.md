...
脚本只识别独占一行、数字已经由上游长度检查器确认的标准句式：`全文约 N 字，预计阅读 M 分钟`。它不重新估算字数和时间，也不把相似正文误判为提示。

| 平台 | `body.md` | `body.html` |
|---|---|---|
| 微信公众号 | 保留普通文本行 | 13px 浅暖色提示块、窄暖色左边线和小内边距 |
| 知乎 | 保留普通文本行 | 14px 浅灰色次级提示段 |
| CSDN | 转换为原生 Markdown 斜体行 | 14px 浅灰色次级提示段 |
| 掘金 | 转换为原生 Markdown 斜体行 | 14px 浅灰色次级提示段 |

四个平台都必须在载荷中保留提示且只出现一次。真实编辑器不保留背景、字号或内边距时，退回平台原生次级文本并在草稿验收中记录，不删除提示，不为它增加标题、引用块或新的内容卡片。
...
- 标准长文阅读提示在载荷中缺失、重复、被提升为标题或因样式降级而被删除。
