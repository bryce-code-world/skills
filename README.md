...
| [广播（broadcast）](./broadcast/SKILL.md) | 按传播效果链分别设计标题、封面、开头、正文和结尾，把完整内部底稿转写为四平台长图文。 |
...
- 把标题、封面、开头、正文和结尾作为五个独立工程，成稿后执行入口、第一屏、推进、最后一屏和效果门禁；
...
```text
使用 $broadcast 从这篇完整技术底稿推导认知缺口和目标效果，用真实同类样本校准标题、封面、开头、正文和结尾的分工，再生成公众号、知乎、CSDN 和掘金传播包。
```
