...
鹰眼当前内置 AI 方向。方向包、全局共享信源目录和五类目标用户注意力画像分别维护；鹰眼只读使用信源，不负责长期维护信源资产。它停止在经过验证的选题，不继续写内部文章或公开平台文章。没有用户确认的交付目录时不会开始正式研究或创建文件。每次把不超过 3 个热点汇总为一个带日期的文件，同日同主题重跑时覆盖当天文件。
