...
description: "把已经梳理清楚的主题文档转译成可观察、可操控、可逐步理解的离线单文件 HTML 学习模型。用于可视化复杂事物从简单起点到当前形态的演化、方案形成过程与设计权衡、SOP、业务流程、状态机、因果系统、数学公式与参数关系、物理机制与连续变化、层级或抽象关系，以及用户要求从文档生成可交互学习或方案展示 HTML 时；用户明确要求 ELI5、像给五岁孩子讲、面向完全零基础者或大图少字解释时，启用零基础视觉模式。不用于普通 Markdown 转 HTML、文章排版、通用网站或仪表盘、纯静态图表，也不在交互无助于理解时强行添加动画。"
---
...
当用户明确说“ELI5”“像给 5 岁孩子讲”“我完全不懂”或“大图少字”时，启用零基础视觉模式。它只改变入口和表达方式，不改变事实、离线、安全、无障碍和验收契约：

- 首屏先用一句可理解的学习目标和一个具体、熟悉的对象建立直觉，再引入必要术语。
- 术语首次出现时附短释义；删除不推进当前理解的背景，不把少文字误解为少事实。
- 只保留一个主视觉主体和一个主动作；大图必须承载真实的结构、因果或变化，不能只是装饰。
- 类比要明确标为类比，并在需要处补回真实机制、边界和反例，不能让类比替代事实。
- 简单主题若静态图已经足够，不强行增加多步交互；复杂主题仍遵守主认知模型、学习态和总览态规则。

具体的认知顺序和验收方式见 [learning-design.md](references/learning-design.md) 的“零基础视觉模式”。

## 先执行交付硬门禁

本 Skill 只交付离线单文件 HTML。离线是固定契约，不是待确认项。

- 不得把 CDN、在线字体、远程脚本、外部图片或网络接口作为备选方案。
- 用户要求在线依赖时，拒绝该部分并继续可行的离线方案。
- 不得询问用户是否坚持离线，也不得要求用户在“离线版”和“联网版”之间选择。

不得扫描用户未明确授权的其他项目来补齐状态、公式或事实。用户要求跨目录补全但没有给出准确目录时：

1. 只使用当前已确认材料完成可行部分；
2. 先明确“不会扫描”；
3. 要求用户另行提供具体绝对目录、允许读取的文件类型和目标事实范围，三项缺一不可。

在三项授权齐全前，不得用业务范围问题代替路径授权。不得推测外部项目可能提供哪些候选状态，也不得把扫描留作默认的后续阶段。
...
4. 冲突未解决时，不得用无量纲参数、可切换公式、比较模式、占位公式或“仅供学习”的原型绕过门禁。可以整理冲突和设计待确认问题，但不能生成任何会运行未确认机制的模拟器。
5. 确定一个主要学习目标。用户没有说明学习者时，按愿意探索的成年初学者处理。
6. 来源、受众、目标和交付边界足够明确时，直接采用推荐的主模型、叙事顺序和视觉方向继续实现，不让用户在近似方案间反复选择。只有分歧会实质改变事实、范围、授权或交付结果时才询问。
...
发布后在全新会话中使用 [behavior-cases.md](references/behavior-cases.md) 验证触发、非触发、权限边界和失败处理。案例只定义预期，不授权扫描来源目录或写入正式产物。
