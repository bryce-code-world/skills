...
6. 从主题的真实对象、工具和表达习惯推导视觉语言，不复用固定页面模板。
7. 将 CSS、JavaScript、SVG、数据和图标全部内联；最终产物只有一个 `.html` 文件。
8. 保留语义结构、键盘操作、可见焦点、窄屏布局、`prefers-reduced-motion` 和 JavaScript 失败时的核心静态内容。
