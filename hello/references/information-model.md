...
正式资料包中，《个人全景档案.md》只做唯一聚合入口和导航门面，详细事实的唯一 owner 在 `权威/` 中的稳定 ID 条目。旧版单文件只作为一次性迁移输入和不可变快照。物理迁移必须旁路、保留来源并经用户确认，不能因目录存在就自动执行；目标 manifest 记录 `package_id`、`subject_id`、主体语言/所有者和默认用途等包级身份元数据，样板不填真实身份。
